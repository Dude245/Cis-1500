...
int main(void) {
    ...
    int ground=0;
       char hword[400] = {""};
        char guessed[100] = "";
    int r=0;
    char random[21][7] = {"alumni","pencil","button","leaves","claude",
	                    "debussy","piano","alison","foxy",
                            "table","weekend",
                            "stupid","sock","earth","love",
    ...
    {
    printf("How many characters? (Max 7) \n");
    scanf("%d%*c", &letters);
    while (letters != 99)
    {
        if (letters > 7) {
            printf("Try again... \n");
            printf("How many characters? (Less than 6) \n");
            scanf("%d%*c", &letters);
        } else
            break;
    }
